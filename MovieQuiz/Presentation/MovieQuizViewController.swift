    ...
    private func showNextQuestionOrResult() {
        if currentQuestionIndex == questions.count - 1 {
          //показать результат квиза
            ...
            accuracy = (Float(answers)/Float(counterQuiz))*10.00
            let date = Date()
            let formatter = DateFormatter()
            formatter.dateFormat = "dd.MM.YYYY HH:mm:ss"
            var dateNow : String
            dateNow = "\(formatter.string(from: date as Date))"
            
            if correctAnswer > recordQuiz {
                recordQuiz = correctAnswer
                dateRecord = dateNow
            }
